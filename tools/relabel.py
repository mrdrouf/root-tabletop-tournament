...
SQUARE_PT = 62    # start sizes; a name too wide is CONDENSED first and only then set smaller,
WIDE_PT = 58      # which is what lets long names ("Squires & Disciples") grow at all
SQUEEZE_FLOOR = 0.80
...
def art_from_label(name, wide):
    """Lift the artwork out of the label that is on the board now (already isolated on alpha)."""
    p = os.path.join(ROOT, "assets/labels", CURRENT[name].rsplit("/", 1)[-1])
    im = Image.open(p).convert("RGBA")
    return trim(im.crop((0, 0, 152, im.height)) if wide else im.crop((0, 0, im.width, 224)))
...
SQUARES = [
    ("Autumn Map",   "map_autumn_v8",   ["Autumn"],   lambda: art_topband("assets/src_art/map_autumn.png"),   0.79, 0),
    ("Winter Map",   "map_winter_v8",   ["Winter"],   lambda: art_topband("assets/src_art/map_winter.png"),   0.79, 0),
    ("Lake Map",     "map_lake_v8",     ["Lake"],     lambda: art_topband("assets/src_art/map_lake.png"),     0.79, 0),
    ("Mountain Map", "map_mountain_v8", ["Mountain"], lambda: art_topband("assets/src_art/map_mountain.png"), 0.79, 0),
    ("Marsh Map",    "map_marsh_v8",    ["Marsh"],    lambda: art_topband("assets/src_art/map_marsh.png"),    0.79, 0),
    ("Gorge Map",    "map_gorge_v8",    ["Gorge"],    lambda: art_topband("assets/src_art/map_gorge.png"),    0.79, 0),

    ("Standard Deck",              "deck_base_v8",    ["Base Deck"],           lambda: art_topband("assets/src_art/deck_base.png"),    0.755, 6),
    ("Exiles and Partisans Deck",  "deck_exiles_v8",  ["Exiles & Partisans"],  lambda: art_topband("assets/src_art/deck_exiles.png"),  0.755, 6),
    ("Squires and Disciples Deck", "deck_squires_v8", ["Squires & Disciples"], lambda: art_topband("assets/src_art/deck_squires.png"), 0.755, 6),

    ("ThemeArt",      "theme_art_v8",          ["Theme"],          lambda: art_topband("assets/src_art/theme.png"),                 0.735, 8),
    ("RankedArt",     "four_player_draft_v8",  ["4-Player Draft"], lambda: art_crop_aspect("assets/images/ranked.png", 1.33, 210),       0.735, 8),
    ("FourBoardsArt", "four_player_setup_v8",  ["4-Player Setup"], lambda: art_crop_aspect("assets/images/4players.png", 1.33),     0.735, 8),
]
...
WIDES = [
    ...
    ("Faction Selector Tool", "faction_selector_v8", ["Faction", "Selector"],
     lambda: Image.open(os.path.join(ROOT, "assets/src_art/faction_selector_new.png")).convert("RGBA")),
    ("Bat Bungler",        "bat_bungler_v8",         ["The Bat", "Bungler"],  lambda: art_from_label("Bat Bungler", True)),
    ("Mob Lobber",         "mob_lobber_v8",          ["Mob", "Lobber"],       lambda: art_from_label("Mob Lobber", True)),
    ("Koffin Keeper",      "koffin_keeper_v8",       ["Koffin", "Keeper"],    lambda: art_from_label("Koffin Keeper", True)),
    ("Vagabond Cards",     "vagabond_cards_v8",      ["Vagabond", "Cards"],   lambda: trim(Image.open(os.path.join(ROOT, "assets/icons_fit/vbcards.png")).convert("RGBA"))),
    ("Landmarks",          "landmarks_v8",           ["Landmarks"],           lambda: art_alpha_topband("assets/icons_fit/landmarks.png", 0.70)),
    ("FivePlayerSetupArt", "five_player_setup_v8",   ["5-Player", "Setup"],   lambda: art_crop_aspect("assets/images/5players.png", 0.853)),
    ("FivePlayerArt",      "five_player_draft_v8",   ["5-Player", "Draft"],   lambda: art_crop_aspect("assets/images/5players.png", 0.853)),
    ("Marsh5PLabel",       "five_players_marsh_v8",  ["5-Players", "Marsh"],  lambda: art_topband("assets/upload/map_marsh.png")),
    ("FactionCardsArt",    "faction_cards_v8",       ["Faction", "Cards"],    lambda: art_from_label("FactionCardsArt", True)),
    ("CreditsBtnArt",      "credits_button_v8",      ["Credits"],             lambda: None),
]
