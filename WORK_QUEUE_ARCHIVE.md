...
### Setup and placement
### Buttons and real estate
- [x] **Map buttons need a wipe warning: "This will reset the current map."** DONE 2026-09-06. All
      six map buttons now go through rttArmMap -> rttArmOrGo, the same arm/commit path the setup
      buttons use: first click swaps the art to a red warning and starts the 3s auto-revert, second
      click commits, and a CLEAN table still places at once (rttWouldWipe takes an isMap flag and
      tests "Map Object" instead of the faction tags).
      The art was NOT generated by any tool -- relabel.py has no Wipe row -- so it was measured off
      wipe_confirm_sq_v3 and reproduced: cream #F7EED6 on a 4px black stroke, transparent 272x272,
      ink inside 240px, Luminari. The existing one reads "This will / reset all / factions." -- read
      off the pixels, not guessed -- three lines at 53pt on an 80px pitch.
      The maintainer's wording does not fit that: "current map." is 329px at 53pt against a 240px
      budget. Offered three ways; he chose THREE LINES AT SMALLER TYPE, so it is "This will / reset
      the / current map." at 39pt on the faction warning's own 13/93/173 line positions. Same shape
      and rhythm, smaller text. Published as wipe_confirm_map_79250013.png and registered in the
      BOARD's CustomUIAssets as WipeConfirmMapArt.
      rttArmOrGo now carries the CLICKING PLAYER through to the commit. makeMap swallows clicks while
      busy and clears RTT_5P_MARSH only for a human click, so committing with nil would have kept
      5-player mode alive through a map change -- the bug fixed the same morning, reintroduced by its
      own fix. The test asserts that specifically.
### Gizmo (the maintainer will iterate; ask before changing behaviour)
### Housekeeping the maintainer flagged
### From this session, still unverified
## AWAITING A TEST AT THE TABLE
