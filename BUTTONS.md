...
kind, Acclaim two to a stack, a Tunnel to its own spot. A relic is never turned: it squares up to its
row and keeps whatever face it shows, set again once it has landed; the Keepers board's relic snaps
carry no rotation any more, so they do not turn it either. It asks no permission and does not care who
...
- **Two buttons are not tools**: `x` (`deleteThis`, top right) removes the setup board itself, and
  ...
  *"This clears everything."* and turns it the armed red, a second within three seconds does it. On a
  ...
  anything tagged `Table Piece` or `Landmark Object`, the Master Instructions and, in a save that
  still has one, the Flex Table Control -- `rttClearAllTakes` is that rule, and `rttWouldWipe` asks
  it, so the warning and the wipe can never disagree.
- **The Flex Table Control is gone** (2026-09-23, "remove the table control panel, the red button at
  the corner of the table"). It was the base mod's table-scaling widget; the table pieces are locked
  in the save and need nothing from it.
