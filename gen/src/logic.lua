...
RTT_LAID_TINT = { r = 0.9765, g = 0.9020, b = 0.7333 }   -- RTT_CREAM #F9E6BB
RTT_LAID = {}      -- [guid] = { rot = {x,y,z}, col = {r,g,b} } as it stood before the key

-- Down and locked in the cream. Nothing else in the mod paints a warrior this colour: the nearest is
-- the Duchy's own beige, 0.085 away in green, four times the tolerance below.
function rttIsLaid(o)
  ...
  local c = nil
  pcall(function() c = o.getColorTint() end)
  if c == nil or c.r == nil then return false end
  return math.abs(c.r - RTT_LAID_TINT.r) < 0.02
     and math.abs(c.g - RTT_LAID_TINT.g) < 0.02
     and math.abs(c.b - RTT_LAID_TINT.b) < 0.02
end
...
function rttGizmoLay(color)
  ...
  if rttIsLaid(hovered) then                            -- second press: stand it back up
    ...
    pcall(function()
      ...
      if was ~= nil then
        ...
        hovered.setColorTint({ r = was.col[1], g = was.col[2], b = was.col[3] })
      else
        -- no record (a table this board did not lay it on): stand it up and clear the tint. The
        -- faction colour of a warrior whose texture carries it is unchanged by this.
        hovered.setRotation({ 0, ry, 0 })
        hovered.setColorTint({ r = 1, g = 1, b = 1 })
      end
    end)
    RTT_LAID[guid] = nil
    return
  ...
  pcall(function() c = hovered.getColorTint() end)
  if r == nil then return end
  if c == nil or c.r == nil then c = { r = 1, g = 1, b = 1 } end
  RTT_LAID[guid] = { rot = { r.x, r.y, r.z }, col = { c.r, c.g, c.b } }
  pcall(function()
    hovered.setRotation({ 90, r.y, r.z })              -- tips forward, away from the player
    hovered.setColorTint(RTT_LAID_TINT)
    hovered.setLock(true)
  end)
end
