...
function rttSeatRecord()
  ...
  for _, i in ipairs(rttSeatOrderIdx()) do
    ...
    out.seats[#out.seats + 1] = {
      ...
      key = s.key or ((s.faction ~= nil) and rttFactionKey(s.faction) or ""),
      owner = s.owner or "",
    }
  end
  return out
...
RTT_TURN_PANEL_JSON = [====[{"Name":"Custom_Tile","Transform":{"posX":0.0,"posY":11.56,"posZ":0.0,"rotX":0.0,"rotY":180.0,"rotZ":0.0,"scaleX":9.4,"scaleY":1.0,"scaleZ":9.4},"Nickname":"Turn Panel","Description":"Round, turn clock, and the two start-of-game buttons.","GMNotes":"","ColorDiffuse":{"r":1.0,"g":1.0,"b":1.0},"Locked":true,"Grid":true,"Snap":true,"IgnoreFoW":false,"MeasureMovement":false,"DragSelectable":true,"Autoraise":true,"Sticky":true,"Tooltip":true,"GridProjection":false,"HideWhenFaceDown":false,"Hands":false,"CustomImage":{"ImageURL":"https://cdn.jsdelivr.net/gh/mrdrouf/root-tabletop-tournament@main/assets/labels/turn_panel_0d074551.png","ImageSecondaryURL":"","ImageScalar":1.0,"WidthScale":0.0,"CustomTile":{"Type":0,"Thickness":0.1,"Stackable":false,"Stretch":true}},"LuaScript":"-- RTT TURN PANEL -- replaces the old clock and counter.\n-- Shows the ROUND the box score is on and a clock for the turn in progress, and carries the two\n-- start-of-game buttons. It owns no game state: the round comes from the sheet (obj.call \"rttRound\")\n-- so the two can never disagree, and the per-player turn LENGTHS are recorded by the sheet itself.\n\n-- Geometry, in image fractions -> tile local coords. The art is 1000x820 and the tile takes the\n-- image's aspect (WidthScale 0), so x runs -1..+1 across the width and z runs -1/A..+1/A down the\n-- height. Every position below is \"where it sits in the picture\", so moving art moves the buttons.\nIMG_W, IMG_H = 1000, 820\nlocal function LX(px) return 2 * px / IMG_W - 1 end\nlocal function LZ(py) return (2 * py / IMG_H - 1) / (IMG_W / IMG_H) end\n\nPANEL_START = nil          -- os.time when the turn in progress began; nil = clock not started\nPANEL_DEALT = false        -- the deal button removes itself once used\nPANEL_TURN  = nil          -- the colour whose turn we are timing\n\nfunction onSave()\n  return JSON.encode({ s = PANEL_START, d = PANEL_DEALT, t = PANEL_TURN })\nend\n\nfunction onLoad(state)\n  pcall(function()\n    if state ~= nil and state ~= \"\" then\n      local d = JSON.decode(state)\n      if type(d) == \"table\" then\n        PANEL_START = d.s; PANEL_DEALT = (d.d == true); PANEL_TURN = d.t\n      end\n    end\n  end)\n  buildPanel()\n  Wait.time(panelTick, 0.25, -1)\nend\n\nfunction sheet()\n  local t = getObjectsWithTag(\"RTT BoxScore\")\n  return (t ~= nil and #t > 0) and t[1] or nil\nend\n\nfunction buildPanel()\n  self.clearButtons()\n  -- the two READOUTS are buttons only so their label can be redrawn; they are invisible and inert\n  self.createButton({ click_function = \"panelNop\", function_owner = self, label = \"-\",\n    position = { LX(500), 0.2, LZ(222) }, width = 0, height = 0, font_size = 260,\n    font_color = { 0.10, 0.08, 0.05 } })\n  self.createButton({ click_function = \"panelNop\", function_owner = self, label = \"0:00\",\n    position = { LX(500), 0.2, LZ(502) }, width = 0, height = 0, font_size = 200,\n    font_color = { 0.10, 0.08, 0.05 } })\n  if not PANEL_DEALT then\n    self.createButton({ click_function = \"panelDeal\", function_owner = self, label = \"DEAL 5 CARDS\",\n      position = { LX(500), 0.2, LZ(650) }, width = 620, height = 110, font_size = 78,\n      color = { 0.86, 0.78, 0.60 }, font_color = { 0.10, 0.08, 0.05 },\n      tooltip = \"Deal five cards to every seated player\" })\n  end\n  self.createButton({ click_function = \"panelStart\", function_owner = self, label = \"START TURN 1\",\n    position = { LX(500), 0.2, LZ(768) }, width = 620, height = 110, font_size = 78,\n    color = { 0.80, 0.68, 0.42 }, font_color = { 0.10, 0.08, 0.05 },\n    tooltip = \"Start the clock and set the box score to round 1, first seat\" })\nend\n\nfunction panelNop() end\n\n-- INDEX MATTERS: clearButtons/createButton order above fixes them. 0 = round, 1 = clock, then the\n-- deal button (when present) and start. editButton by index is the only way to relabel in place.\nfunction panelTick()\n  local r = \"-\"\n  pcall(function()\n    local s = sheet()\n    if s ~= nil then local v = s.call(\"rttRound\"); if v ~= nil then r = tostring(v) end end\n  end)\n  pcall(function() self.editButton({ index = 0, label = r }) end)\n\n  -- the clock follows the TURN, not the panel: a new turn restarts it wherever it came from\n  local cur = nil\n  pcall(function() cur = Turns.turn_color end)\n  if cur ~= nil and cur ~= \"\" and cur ~= PANEL_TURN then\n    PANEL_TURN = cur\n    if PANEL_START ~= nil then PANEL_START = os.time() end\n  end\n  local txt = \"0:00\"\n  if PANEL_START ~= nil then\n    local n = os.time() - PANEL_START\n    if n < 0 then n = 0 end\n    txt = string.format(\"%d:%02d\", math.floor(n / 60), n % 60)\n  end\n  pcall(function() self.editButton({ index = 1, label = txt }) end)\nend\n\n-- START TURN 1: the clock begins HERE, at the moment the table is actually ready, and the sheet is\n-- forced to round 1 / first seat rather than asked to agree.\nfunction panelStart()\n  PANEL_START = os.time()\n  pcall(function()\n    local o = Turns.order\n    if o ~= nil and #o > 0 then Turns.turn_color = o[1] end\n    PANEL_TURN = Turns.turn_color\n  end)\n  pcall(function()\n    local s = sheet()\n    if s ~= nil then s.call(\"rttStartTurnOne\") end\n  end)\n  broadcastToAll(\"Turn 1.\", { r = 0.85, g = 0.75, b = 0.55 })\nend\n\n-- DEAL 5 CARDS: five to every seated player, then the button goes for good.\nfunction panelDeal()\n  if PANEL_DEALT then return end\n  local deck = nil\n  for _, o in ipairs(getObjectsWithTag(\"Deck Object\")) do\n    local ok, n = pcall(function() return o.getQuantity() end)\n    if ok and n ~= nil and n > 20 then deck = o break end\n  end\n  if deck == nil then\n    broadcastToAll(\"Deal 5: no deck on the table.\", { r = 1, g = 0.6, b = 0.2 })\n    return\n  end\n  local n = 0\n  for _, p in ipairs(Player.getPlayers()) do\n    if p.seated and p.color ~= \"Grey\" and p.color ~= \"Black\" then\n      pcall(function() deck.deal(5, p.color) end)\n      n = n + 1\n    end\n  end\n  if n == 0 then\n    broadcastToAll(\"Deal 5: nobody is seated.\", { r = 1, g = 0.6, b = 0.2 })\n    return\n  end\n  PANEL_DEALT = true\n  buildPanel()                                  -- rebuilt without the deal button\n  broadcastToAll(\"Dealt 5 cards to \" .. n .. \" player(s).\", { r = 0.85, g = 0.75, b = 0.55 })\nend\n","LuaScriptState":"","XmlUI":""}]====]
...
function rttSpawnMapExtras()
  ...
  if rttFixture("Turn Panel") ~= nil then return end
  spawnObjectJSON({
