...
function rttGizmoLay(color)
  ...
  if rttIsLaid(hovered) then                            -- second press: stand it back up
    ...
    pcall(function()
      ...
      if was ~= nil then
        ...
        if was.pos ~= nil then
          hovered.setPosition({ was.pos[1], was.pos[2], was.pos[3] })
        end
  ...
  RTT_LAID[guid] = { rot = { r.x, r.y, r.z }, col = { c.r, c.g, c.b }, pos = { p.x, p.y, p.z } }
  pcall(function()
  ...
  Wait.frames(function()
    pcall(function()
      if foot ~= nil then
        local nb, np = hovered.getBounds(), hovered.getPosition()
        local dy = foot - (nb.center.y - nb.size.y / 2)
        hovered.setPosition({ np.x, np.y + dy, np.z })
      end
    end)
