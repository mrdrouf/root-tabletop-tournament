    ...
    for i, s in ipairs(RTT_SEATS or {}) do
      if s ~= nil and s.pos ~= nil then
        ...
        seats[#seats + 1] = { i = i, pos = { s.pos[1], s.pos[2] }, color = s.color,
                              ...
                              -- steamId: captured at the pick and unrecoverable afterwards -- the
                              -- player may be long gone -- so it has to survive a reload with the rest
                              steamId = s.steamId,
...
-- Which pump owns the queue. A pump that wakes holding a stale token has been superseded and stands
-- down without touching anything. See rttSpawnStaggered.
RTT_SPAWN_TOKEN   = nil
...
function rttSpawnStaggered(specs, done, alive)
  ...
  -- WHICH RUN THIS QUEUE BELONGS TO, and which pump owns it.
  --
  -- The tail of this function used to re-arm through rttAfterFrames, which SILENTLY DROPS its
  -- callback once RTT_RUN_ID moves on. So pressing New Game (or any button that starts a run) while
  -- a spawn was still draining killed the pump mid-queue and left RTT_SPAWN_PUMPING true with
  -- entries still in RTT_SPAWN_Q -- and every later call fell straight out at the line above,
  -- appending to a queue nothing would ever drain. Nothing spawned again until the mod was reloaded.
  --
  -- The pump now checks for itself, so it can clean up instead of vanishing.
  local gen, token = RTT_RUN_ID, {}
  RTT_SPAWN_TOKEN = token
  local function pump()
    if RTT_SPAWN_TOKEN ~= token then return end      -- a newer pump owns the queue; leave it alone
    if RTT_RUN_ID ~= gen then                        -- the run this queue belonged to is over
      RTT_SPAWN_Q = {}
      RTT_SPAWN_PUMPING = false
      return
    end
    ...
    if #RTT_SPAWN_Q > 0 then
      -- PLAIN Wait.frames, deliberately: rttAfterFrames would drop this callback on a run change and
      -- take the queue and the busy flag to the grave with it. The generation check at the top of
      -- pump does the same job and leaves the pump able to tidy up after itself.
      Wait.frames(pump, 1)
    else
      RTT_SPAWN_PUMPING = false
    end
...
function rttClearGameObjects()
  ...
  -- ...including anything the abandoned run had queued to spawn. The pump would drop it a frame
  -- later anyway; doing it here means a setup starting in this same frame begins with an empty queue.
  RTT_SPAWN_Q = {}
  RTT_SPAWN_PUMPING = false
...
function rttResetRunState()
  ...
  for _, k in ipairs({ "RTT_SEAT_POS", "RTT_SEAT_COLOR", "RTT_SEAT_PLAYER", "RTT_SEAT_STEAM",
                       "RTT_SEAT_RECORD" }) do
...
-- ...AND THEIR STEAM ID, read at the same moment and in the same way.
--
...
--
-- THE PICK IS THE ONLY RELIABLE MOMENT. TTS hands out a steam_id for a SEATED player and no other
-- way -- there is no lookup by name, and none at all for a colour nobody is in. The box score used to
-- go looking for one on its own poll, which finds the person sitting in that row's colour NOW: right
-- while everyone is still at the table, empty for anyone who left before the upload, and quietly
-- WRONG for a colour somebody else has since taken. At the pick we know exactly whose hand did it.
--
-- One player picking several factions in a row gives all of those rows their id, which is what he
-- asked for and what a tournament host setting up seats actually wants.
function rttSteamIn(color)
  if color == nil or color == "" then return nil end
  local id = nil
  pcall(function()
    for _, pl in ipairs(Player.getPlayers()) do
      if pl.color == color and pl.seated and pl.steam_id ~= nil and tostring(pl.steam_id) ~= "" then
...
-- Same rule for the id: a real one replaces an older one (the seat changed hands), nothing never
-- replaces something (the picker was not seated, or TTS had no id to give).
function rttSetSeatSteam(seat, id)
  if seat == nil or id == nil or id == "" then return seat and seat.steamId or nil end
  seat.steamId = id
  return id
end
...
function rttPublishSeats()
  ...
  for _, e in ipairs(rec.seats) do
    ...
    if k ~= "" then
      ...
      if e.steam ~= "" then sid[k] = e.steam end
    end
  end
  ...
  -- faction -> the steam id of whoever picked it. A map of its own rather than a field inside
  -- RTT_SEAT_RECORD alone, because that is how the sheet already reads the owner NAME, and an older
  -- box-score bake that knows nothing about this simply never asks for it.
  pcall(function() Global.setVar("RTT_SEAT_STEAM", JSON.encode(sid)) end)
...
function rttPlaceFaction(faction, cx, cz, flip, color, isDraft, category, rotationY, pickerColor, seatHand)
  ...
  if pickerColor ~= nil and pickerColor ~= "" then
    ...
    rttSetSeatSteam(seat, rttSteamIn(pickerColor))
  end
  rttPublishSeats()
...
function rttBagOfMap()
    ...
    for _, cat in pairs(EVERYTHING) do
      for _, def in pairs(cat) do
        if type(def) == "table" and def['data'] ~= nil then
          for _, v in ipairs(def['data']) do
            -- ANY CONTAINER, NOT ONLY ONE CALLED "... Supply". Maintainer, 2026-09-13: numpad 2
            -- "does not work with relics ... it should work with everything". The twelve relics live
            -- inside a Bag nicknamed "Relics", which this pattern did not match, so "Relic" was in no
            -- bag map and had no home slots either -- it never spawns as a loose object, so nothing
            -- ever recorded one. rttTokenEligible therefore said no and the kind could not even be
            -- CHOSEN, let alone handed out.
            --
            -- The first Nickname in a blueprint is the container's own and the rest are its contents,
            -- which is what the old pattern relied on; this keeps that and drops the name test.
            local bag = v.json:match('"Name":%s*"[^"]*Bag[^"]*".-"Nickname":%s*"([^"]*)"')
                     or v.json:match('"Nickname":%s*"([^"]*Supply)"')
  ...
  -- `built` is the SCAN's own pcall, not a second one wrapped round the check. The first version of
  -- this guard read `pcall(function() return next(m) end)`, which asks whether looking at the table
  -- throws -- it never does -- so a scan that died halfway was still cached as complete. Found by an
  -- adversarial review on 2026-09-14, an hour after it was written.
  if built and next(m) ~= nil then RTT_BAG_OF = m end
  return m
end
...
--
-- The Keepers' three waystations are "Tablet/Figure Waystation", "Jewelry/Tablet Waystation" and
-- "Figure/Jewelry Waystation" -- one object each, and three names. Every slot lookup in this file
-- matches on the exact name, so each waystation had a row of ONE: its own spawn spot. Send it home
-- and it goes there whether or not another waystation is already sitting on it, because the
-- occupancy test only ever looked for a piece of the SAME name. Three pieces, three rows of one, and
-- nothing anywhere aware that they form a row together -- so they pile up.
--
-- KEYED ON THE LAST WORD because that is what the three names have in common and it is how the game
-- itself names them: the varying part says which two relics the waystation serves, and the shared
-- part says what it is. A single-word name is its own family and behaves exactly as before.
RTT_HOME_FAMILY = { ["Waystation"] = true }

-- The family a name belongs to, or the name itself when it is in none.
function rttHomeFamily(name)
  local last = tostring(name or ""):match("(%S+)%s*$")
  if last ~= nil and RTT_HOME_FAMILY[last] then return last end
  return name
end
...
function rttOrderSlots(out, name)
  ...
  -- COLUMNS AND ROWS ARE WORKED OUT FIRST, and the sort then compares those numbers.
  --
  -- This used to compare the raw coordinates with a 0.2 tolerance inside the comparator itself, and
  -- "within 0.2 of each other" IS NOT AN ORDERING: it is not transitive. Three slots drifting a
  -- seventh of a unit apart -- x 0.00 / 0.15 / 0.30 with z falling away -- gave a < b, b < c AND
  -- c < a. Lua is entitled to raise "invalid order function for sorting" on that, and a throw here
  -- takes numpad 0 and numpad 2 down with it; short of throwing it just returns an arbitrary order,
  -- so a row could fill in a different sequence on different presses.
  --
  -- Found by an adversarial review, 2026-09-14, and reproduced: it needs slots spaced closer than
  -- the tolerance but not identical, which no row in the mod happens to have today. It is a mine
  -- lying in the path of the next row anybody adds.
  --
  -- Clustering once, up front, is transitive by construction -- each slot gets an integer, and
  -- integers are a total order -- and it reproduces exactly the grouping the tolerance intended,
  -- because a real column is 1.4 or more from the next.
  local function cluster(axis)
    local idx, order = {}, {}
    for i, h in ipairs(out) do order[i] = h end
    table.sort(order, function(a, b) return a.p[axis] < b.p[axis] end)
    local n, prev = 0, nil
    for _, h in ipairs(order) do
      if prev == nil or (h.p[axis] - prev) > 0.2 then n = n + 1 end
      idx[h] = n
      prev = h.p[axis]
    end
    return idx
  end
  local colOf, rowOf = cluster(1), cluster(3)

  table.sort(out, function(a, b)
    if per ~= nil then                       -- stacks: nearest row first, then right to left, then up
      if rowOf[a] ~= rowOf[b] then return rowOf[a] * s < rowOf[b] * s end
      if colOf[a] ~= colOf[b] then return colOf[a] * s > colOf[b] * s end
      return a.p[2] < b.p[2]
    end
    if colOf[a] ~= colOf[b] then return colOf[a] * s * lf > colOf[b] * s * lf end
    if rowOf[a] ~= rowOf[b] then return rowOf[a] * s < rowOf[b] * s end
    return a.p[2] < b.p[2]
  end)
  return out
...
-- Every recorded home slot belonging to one family, in fill order.
...
-- the family -- which is the whole point for the waystations, whose three names differ. For every
-- other piece the family IS the name and this behaves exactly as it always did.
--
-- NOT "anything at all", which was the first version of this and was wrong: a slot sits a fraction
-- above the board it is printed on, well inside the y tolerance, so counting any object would have
-- read every slot in the game as permanently taken.
