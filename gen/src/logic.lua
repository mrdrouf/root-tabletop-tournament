...
function makeFaction(player,value,id)
  ...
  -- order rows by SEAT (clockwise, right->left) instead of pick order. Keyed by player colour.
  RTT_SEAT_POS = RTT_SEAT_POS or {}
  do local cp = self.getPosition(); RTT_SEAT_POS[player.color] = { cp.x, cp.z } end
  Global.setVar("RTT_SEAT_POS", RTT_SEAT_POS)
...
function rttCoordFaction(args)
  ...
  RTT_SEAT_POS = RTT_SEAT_POS or {}                    -- seat position for the box-score seat order
  RTT_SEAT_POS[s.color or args.color] = { bp.x, bp.z }
  Global.setVar("RTT_SEAT_POS", RTT_SEAT_POS)
